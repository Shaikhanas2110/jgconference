...
const mobileSubLinkClasses = ({ isActive }) =>
  `block pl-11 pr-6 py-3 text-sm duration-300 ${
    isActive ? "text-red-700 font-semibold" : "text-gray-600 hover:text-red-700"
  }`;
    ...
    <header className="bg-white shadow-sm sticky top-0 z-50 border-b border-yellow-300">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-6">
        ...
        <ul className="hidden lg:flex items-center gap-10 text-gray-800 font-medium">
          <Link
            to="/"
            className={`pb-1 border-b-2 duration-300 ${
              isHomeActive
                ? "text-red-700 font-semibold border-red-700"
                : "text-gray-800 border-transparent hover:text-red-700"
            }`}
          ...
          <li className="relative list-none" ref={dropdownRef}>
            <button
              className={`flex items-center gap-1 pb-1 border-b-2 duration-300 font-medium ${
                isCallForPaperActive
                  ? "text-red-700 font-semibold border-red-700"
                  : "text-gray-800 border-transparent hover:text-red-700"
              }`}
            ...
            <div
              className={`absolute top-12 left-0 bg-white rounded-xl shadow-2xl w-72 overflow-hidden z-40 transition-all duration-300 ease-in-out transform origin-top-left ${
                dropdown
      ...
      {/* Mobile Menu Drawer */}
      <div
        className={`lg:hidden fixed top-0 right-0 h-full w-[85%] max-w-sm bg-white z-50 shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${
        ...
        <nav className="flex-1 overflow-y-auto">
          ...
          <div className="border-b border-gray-50">
            <button
              ...
              className={`w-full flex items-center justify-between px-6 py-4 text-base duration-300 ${
                isCallForPaperActive
            ...
            >
              ...
              <FaChevronDown
                ...
                className={`text-gray-400 transition-transform duration-300 ${
                  mobileCFPOpen ? "rotate-180 text-red-700" : ""
                }`}
              />
            </button>

            <div
              className={`overflow-hidden bg-gray-50/70 transition-all duration-300 ease-in-out ${
                mobileCFPOpen ? "max-h-96" : "max-h-0"
              }`}
            >
